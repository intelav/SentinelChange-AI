...
# FIX: Import the specific module from your project where ChangeStarBiSup is defined.
# This ensures the model is registered with the 'ever' framework.
...
def remove_small_objects(mask, min_size=20):
...
def generate_sam_labels(changestar_model, sam_predictor, processed_data_dir, device):
    ...
    and SAM for precise segmentation.
    """
    ...
    # --- ADDED: Counters and lists for summary ---
    ...
    logging.info(f"Starting to generate SAM-based labels for {len(image_files)} image pairs...")

    with torch.no_grad():
        for filename in tqdm(image_files, desc="Generating Labels"):
            ...
            if coarse_map.sum() < 10:  # Threshold for min change area
                logging.info("No significant change detected by ChangeStar. Saving empty label.")
                empty_label = Image.fromarray(np.zeros((1024, 1024), dtype=np.uint8), 'L')
                label_path = os.path.join(label_dir, filename)
                if not os.path.exists(label_path):
                    empty_label.save(label_path)

                # --- ADDED: Update no-change counters ---
                no_change_count += 1
            ...
            # OPTIMIZATION: Remove small, noisy objects from the final label
            cleaned_label = remove_small_objects(final_label, min_size=20)
            logging.info(
                f"Final label created. Pixels before cleaning: {final_label.sum()}, Pixels after cleaning: {cleaned_label.sum()}")

            # --- ADDED: Update change/no-change counters based on cleaned label ---
    ...
    # --- ADDED: Final summary and report generation ---
    ...
    summary_path = os.path.join(processed_data_dir, 'change_detection_summary.txt')
    try:
        with open(summary_path, 'w') as f:
            f.write("--- Change Detection Summary ---\n")
...
if __name__ == '__main__':
    # --- CONFIGURATION ---
    # UPDATED: Point to the SYSU-CD fine-tuned model for more general change detection
    ...
    # RECOMMENDATION: Use the 'vit_h' (Huge) model for the best quality labels.
    # You will need to download the 'sam_vit_h_4b8939.pth' checkpoint file.
    SAM_CHECKPOINT_PATH = './sam_vit_h_4b8939.pth'
    SAM_MODEL_TYPE = 'vit_h'

    PROCESSED_DATA_DIR = '../data/MyCustomCD_Dataset'
    # ---
